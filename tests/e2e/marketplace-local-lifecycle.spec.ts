...
test.describe("KForge Marketplace local package lifecycle in production", () => {
  ...
  test("inspects, installs, health-checks, runs, updates, and uninstalls the verified bundled extension through the product surface", async ({ page }) => {
    ...
    await expect(details.getByRole("button", { name: "Health check", exact: true })).toBeVisible();
    ...
    await expect(page.locator(".kf-active-surface")).toContainText(/Health check passed|Local package health verified/i);
    ...
    await expect(page.locator(".kf-active-surface")).toContainText(/Local package run verified|JSON Inspector/i);
    ...
    await expect(details.getByRole("button", { name: "Install local package", exact: true })).toBeVisible();
