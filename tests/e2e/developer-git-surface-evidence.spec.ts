...
test.describe("KForge developer workbench and Git evidence", () => {
  ...
  test("runs only detected project commands and keeps structured local observability and Git evidence", async ({ page }) => {
    ...
    const testsWorkbench = page.getByRole("region", { name: "KForge Tests Workbench", exact: true });
    await expect(testsWorkbench).toBeVisible();
    await expect(testsWorkbench).toHaveAttribute("data-test-run-state", "NOT_RUN_THIS_SESSION");
    ...
    await testsWorkbench.getByRole("button", { name: "Run detected tests", exact: true }).click();
    expect((await testResponse).ok()).toBeTruthy();
    await expect(testsWorkbench).toHaveAttribute("data-test-run-state", "PASSED", { timeout: 60_000 });
    ...
    const buildWorkbench = page.getByRole("region", { name: "KForge Build Workbench", exact: true });
    await expect(buildWorkbench).toBeVisible();
    await expect(buildWorkbench).toHaveAttribute("data-build-run-state", "NOT_RUN_THIS_SESSION");
    ...
    await buildWorkbench.getByRole("button", { name: "Run detected build", exact: true }).click();
    expect((await buildResponse).ok()).toBeTruthy();
    await expect(buildWorkbench).toHaveAttribute("data-build-run-state", "PASSED", { timeout: 60_000 });
