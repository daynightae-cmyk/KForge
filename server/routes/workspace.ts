...
import { checkPreviewHealth, getPreviewStatus, inspectPreviewCapability, inspectPreviewDocument, restartPreview, startPreview, stopPreviewAndWait, waitForPreviewHealth } from "../services/previewRuntime";
import { checkTopologyHealth, getRuntimeTopology, getTopologySession, restartTopology, restartTopologyService, startTopology, startTopologyService, stopTopology, stopTopologyService } from "../services/topologyRuntime";
...
async function completenessIssues(project: ProjectSummary, profile: ProjectProfile) {
  ...
  if (profile.envFiles.length > 0 && !(await pathExists(path.join(project.path, ".env.example")))) issues.push(issue(project.id, "missing-env-example", "low", "completeness", "Environment example is missing", "Environment files were detected without a .env.example template.", { source: "KForge completeness", fixability: "automatic", risk: "safe", suggestion: "Preview the deterministic local template, then confirm creation of .env.example with variable names and blank values only." }));
...
router.get("/projects/:id/preview", async (req, res) => {
  ...
  if (!project) return res.status(404).json({ error: "Project not found in the configured KForge workspace." });
  const profile = await detectProjectProfile(project);
  return res.json({ projectId: project.id, trust: project.trust, capability: inspectPreviewCapability(profile), preview: getPreviewStatus(project.id) });
});

router.get("/projects/:id/topology", async (req, res) => {
  const project = await resolveProject(req.params.id);
  if (!project) return res.status(404).json({ error: "Project not found in the configured KForge workspace." });
  return res.json({ projectId: project.id, trust: project.trust, ...(await getRuntimeTopology(project.id, project.path)) });
});

router.post("/projects/:id/topology/start", async (req, res) => {
  const project = await resolveProject(req.params.id);
  if (!project) return res.status(404).json({ error: "Project not found in the configured KForge workspace." });
  if (project.trust !== "trusted") return res.status(428).json(untrustedProjectError("Topology start"));
  const preview = getPreviewStatus(project.id);
  if (["starting", "running"].includes(preview.state)) return res.status(409).json({ error: `Canonical Preview session ${preview.sessionId || "active"} already owns this project's runtime. Stop it before starting the topology.` });
  try { return res.status(202).json({ projectId: project.id, session: await startTopology(project.id, project.path) }); }
  catch (error) { return res.status(500).json({ error: error instanceof Error ? error.message : "Topology could not start." }); }
});

router.post("/projects/:id/topology/health", async (req, res) => {
  const project = await resolveProject(req.params.id);
  if (!project) return res.status(404).json({ error: "Project not found in the configured KForge workspace." });
  return res.json({ projectId: project.id, session: await checkTopologyHealth(project.id) });
});

router.post("/projects/:id/topology/stop", async (req, res) => {
  const project = await resolveProject(req.params.id);
  if (!project) return res.status(404).json({ error: "Project not found in the configured KForge workspace." });
  if (project.trust !== "trusted") return res.status(428).json(untrustedProjectError("Topology stop"));
  try { return res.json({ projectId: project.id, session: await stopTopology(project.id) }); }
  catch (error) { return res.status(500).json({ error: error instanceof Error ? error.message : "Topology did not stop cleanly." }); }
});

router.post("/projects/:id/topology/restart", async (req, res) => {
  const project = await resolveProject(req.params.id);
  if (!project) return res.status(404).json({ error: "Project not found in the configured KForge workspace." });
  if (project.trust !== "trusted") return res.status(428).json(untrustedProjectError("Topology restart"));
  const preview = getPreviewStatus(project.id);
  if (["starting", "running"].includes(preview.state)) return res.status(409).json({ error: `Canonical Preview session ${preview.sessionId || "active"} already owns this project's runtime. Stop it before restarting the topology.` });
  try { return res.status(202).json({ projectId: project.id, session: await restartTopology(project.id, project.path) }); }
  catch (error) { return res.status(500).json({ error: error instanceof Error ? error.message : "Topology could not restart." }); }
});

router.post("/projects/:id/topology/services/:serviceId/:operation", async (req, res) => {
  const project = await resolveProject(req.params.id);
  if (!project) return res.status(404).json({ error: "Project not found in the configured KForge workspace." });
  if (project.trust !== "trusted") return res.status(428).json(untrustedProjectError("Topology service operation"));
  const operation = req.params.operation;
  if (!["start", "stop", "restart"].includes(operation)) return res.status(400).json({ error: "Unsupported topology service operation." });
  const preview = getPreviewStatus(project.id);
  if (operation !== "stop" && ["starting", "running"].includes(preview.state)) return res.status(409).json({ error: `Canonical Preview session ${preview.sessionId || "active"} already owns this project's runtime. Stop it before starting a topology service.` });
  try {
    const session = operation === "start" ? await startTopologyService(project.id, project.path, req.params.serviceId)
      : operation === "stop" ? await stopTopologyService(project.id, req.params.serviceId)
        : await restartTopologyService(project.id, project.path, req.params.serviceId);
    return res.status(operation === "stop" ? 200 : 202).json({ projectId: project.id, session });
  } catch (error) { return res.status(500).json({ error: error instanceof Error ? error.message : `Service ${operation} failed.` }); }
});
...
router.post("/projects/:id/preview/start", async (req, res) => {
  ...
  const topology = getTopologySession(project.id);
  if (topology && ["STARTING", "RUNNING", "HEALTHY", "DEGRADED"].includes(topology.state)) return res.status(409).json({ error: `Topology session ${topology.id} owns this project's services. Use the selected topology browser entrypoint instead of starting a duplicate Preview runtime.` });
  const profile = await detectProjectProfile(project);
  try {
...
router.post("/projects/:id/preview/inspect", async (req, res) => {
  const project = await resolveProject(req.params.id);
  if (!project) return res.status(404).json({ error: "Project not found in the configured KForge workspace." });
  const route = typeof req.body?.route === "string" ? req.body.route : "/";
  const inspection = await inspectPreviewDocument(project.id, route);
  return res.status(inspection.state === "FAILED" ? 422 : 200).json({ projectId: project.id, inspection });
});
...
router.post("/projects/:id/preview/restart", async (req, res) => {
  ...
  const topology = getTopologySession(project.id);
  if (topology && ["STARTING", "RUNNING", "HEALTHY", "DEGRADED"].includes(topology.state)) return res.status(409).json({ error: `Topology session ${topology.id} owns this project's services. Restart the topology or selected service instead.` });
  const profile = await detectProjectProfile(project);
  try {
