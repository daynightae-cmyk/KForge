...
export async function getOnlineControlCenter(input: {
  ...
  const services: OnlineControlServiceStatus[] = [
    ...
    status("remote-documentation", "Remote Documentation", remoteState(online, true, contacts["remote-documentation"]), "Allowlisted provider documentation registry", null, "REQUIRED", "Provider documents refresh only on explicit request; documentation search reads bounded cache locally.", contacts["remote-documentation"], now),
    ...
    status("security-intelligence", "Security Intelligence", remoteState(online, true, contacts["security-intelligence"]), "OSV.dev advisory adapter and explicit package queries", "https://api.osv.dev", "REQUIRED", "OSV advisories are queried only by explicit package/version checks. Opening Security surfaces performs no request.", contacts["security-intelligence"], now),
    ...
    status("updates", "Updates", remoteState(online, true, contacts.updates), "KForge GitHub Releases update-discovery adapter", "https://api.github.com", "REQUIRED", "Release discovery runs only on explicit refresh; availability is a catalog fact and trusted install stays blocked by checksum/signature policy.", contacts.updates, now),
  ];
  return {
