...
export default function KForgeWorkspace() {
  ...
  const setPlatformMode = async (mode: LocalPlatformStatus["mode"]) => {
    if (mode === "online-optional" && !window.confirm("Enable Online Optional mode? Core KForge stays local, but clone, pull, push, and confirmed model downloads may contact external services only after you trigger them.")) return;
    try {
