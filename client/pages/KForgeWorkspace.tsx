...
function GitHubCenterPanel({ project, view, onlineOptional }: { project: ProjectSummary; view: "GitHub" | "Pull requests" | "Issues" | "Actions" | "Releases"; onlineOptional: boolean }) {
  ...
  const refresh = async () => { if (!onlineOptional) { setMessage("Enable Online Optional before loading GitHub metadata. Local Git remains available offline."); return; } try { const response = await fetch(`/api/workspace/projects/${project.id}/github`); const payload = await response.json() as GitHubData; if (payload.connection || payload.checks) setData(payload); if (!response.ok) throw new Error(payload.error || payload.connection?.reason || "GitHub metadata is unavailable."); setData(payload); setMessage(""); } catch (cause: unknown) { setMessage(cause instanceof Error ? cause.message : "GitHub request failed."); } };
