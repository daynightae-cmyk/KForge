...
function GitHubCenterPanel({ project, view, onlineOptional }: { project: ProjectSummary; view: "GitHub" | "Pull requests" | "Issues" | "Actions" | "Releases"; onlineOptional: boolean }) {
  ...
  useEffect(() => { setData(null); setMessage(onlineOptional ? `Ready to load ${view} metadata after an explicit refresh.` : `${view} metadata stays disabled in Offline Mode.`); }, [project.id, onlineOptional, view]);
  ...
  const cards = view === "Pull requests" ? pulls.slice(0, 20).map((pull) => <article className="kf-provider-card" key={`pr-${pull.number}`}><div><strong>#{pull.number} {pull.title}</strong><span>Pull request</span></div></article>) : view === "Issues" ? issues.slice(0, 20).map((issue) => <article className="kf-provider-card" key={`issue-${issue.number}`}><div><strong>#{issue.number} {issue.title}</strong><span>Issue</span></div></article>) : view === "Actions" ? runs.slice(0, 20).map((run) => <article className="kf-provider-card" key={run.id}><div><strong>{run.name}</strong><span>{run.conclusion || run.status}</span></div></article>) : view === "Releases" ? releases.slice(0, 20).map((release) => <article className="kf-provider-card" key={`release-${release.id}`}><div><strong>{release.name || release.tag_name || "Untitled release"}</strong><span>{release.draft ? "draft" : release.prerelease ? "pre-release" : "release"}</span></div><small>{release.tag_name || "No tag reported"}</small></article>) : [];
  return <section className="kf-capability-panel"><div className="kf-card-heading"><div><Github size={17} /><h3>{view}</h3></div><button disabled={!onlineOptional} onClick={() => void refresh()}>Refresh {view}</button></div><p className="kf-capability-copy">Execution: REMOTE read · Network: REQUIRED · Data: repository metadata only · Secrets: handled by the configured GitHub CLI and never rendered. Remote writes remain unavailable from this read-only surface.</p>{message && <StatusMessage>{message}</StatusMessage>}{data && <><div className="kf-hardware-grid"><span><strong>Repository</strong>{data.repository?.full_name || data.slug || "Unavailable"}</span><span><strong>Default branch</strong>{data.repository?.default_branch || "Unavailable"}</span><span><strong>{view === "Pull requests" ? "Open pull requests" : view === "Issues" ? "Open issues" : view === "Actions" ? "Workflow runs" : view === "Releases" ? "Releases" : "Remote state"}</strong>{view === "Pull requests" ? pulls.length : view === "Issues" ? issues.length : view === "Actions" ? runs.length : view === "Releases" ? releases.length : "CONNECTED"}</span></div>{view === "GitHub" ? <div className="kf-provider-grid"><article className="kf-provider-card"><div><strong>Pull requests</strong><span>{pulls.length}</span></div><small>Open remote pull-request metadata</small></article><article className="kf-provider-card"><div><strong>Issues</strong><span>{issues.length}</span></div><small>Open remote issue metadata</small></article><article className="kf-provider-card"><div><strong>Actions</strong><span>{runs.length}</span></div><small>Latest workflow-run metadata</small></article><article className="kf-provider-card"><div><strong>Releases</strong><span>{releases.length}</span></div><small>Published/draft release metadata</small></article></div> : cards.length ? <div className="kf-provider-grid">{cards}</div> : <p className="kf-capability-copy">No {view.toLowerCase()} were returned by the configured GitHub source.</p>}</>}</section>;
}
...
function OnlineHubPanel({ initialView, project, platform }: { initialView: OnlineHubView; project: ProjectSummary; platform?: LocalPlatformStatus }) {
  ...
  const load = async () => {
    try {
      const [response, taskResponse] = await Promise.all([fetch("/api/workspace/marketplace"), fetch("/api/workspace/tasks")]);
      const payload = await response.json();
