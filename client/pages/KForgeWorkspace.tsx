...
function CodeUnderstandingPanel({ project }: { project: ProjectSummary }) {
  const [data, setData] = useState<{ profile?: { framework: string[]; languages: string[]; sourceRoots: string[]; testRoots: string[]; workspaceKind: string; runtimeEntrypoint?: string; sourceFileCount: number; totalFileCount: number }; graph?: { summary: { files: number; imports: number; exports: number; symbols: number; dependencies: number; routes: number; apis: number; tests: number } } } | null>(null);
...
function ArchitecturePanel({ project }: { project: ProjectSummary }) {
  const [data, setData] = useState<{ generatedAt: string; modules: Array<{ name: string; files: number }>; apiBoundaries: Array<{ path: string; owner?: string }>; routeBoundaries: Array<{ path: string; owner?: string }>; directCycles: string[][]; dependencyCycles: string[][]; duplicatedResponsibilities: Array<{ symbol: string; kind: string; files: string[]; evidence: string }>; languageAdapters: Array<{ language: string; files: number; state: string; adapter: string; reason: string }>; highCoupling: Array<{ file: string; dependents: number }>; limitations: string[] } | null>(null);
