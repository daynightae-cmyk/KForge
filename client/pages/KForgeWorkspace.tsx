...
export const CAPABILITY_RENDERER_IDS: Record<string, string> = {
  ...
  "Settings": "LocalPlatformPanel", "Trust": "TrustPanel", "Permissions": "PermissionsPanel", "Storage": "StoragePanel", "Offline / Online": "LocalPlatformPanel", "System diagnostics": "SystemDiagnosticsPanel",
};
...
function CapabilitySurface({ activeNav, project, projects, scan, tasks, results, platform, onPlatformModeChange, onOpenProject, onRun, onTaskControl, onTrust, onNavigate }: { activeNav: string; project: ProjectSummary; projects: ProjectSummary[]; scan?: ProjectScan; tasks: TaskItem[]; results?: Partial<Record<WorkspaceAction, CommandResult>>; platform?: LocalPlatformStatus; onPlatformModeChange: (mode: LocalPlatformStatus["mode"]) => void; onOpenProject: () => void; onRun: (action: WorkspaceAction) => void; onTaskControl: (task: TaskItem, control: "cancel" | "retry" | "resume" | "rollback") => void; onTrust: () => void; onNavigate: (label: string) => void }) {
  const renderers: Record<string, ReactNode> = {
    ...
    "Git": <GitCenterPanel project={project} />, "Branches": <GitCenterPanel project={project} />, "Commits": <GitCenterPanel project={project} />, "GitHub": <GitHubCenterPanel project={project} onlineOptional={platform?.mode === "online-optional"} />, "Pull requests": <GitHubCenterPanel project={project} onlineOptional={platform?.mode === "online-optional"} />, "Issues": <GitHubCenterPanel project={project} onlineOptional={platform?.mode === "online-optional"} />, "Actions": <GitHubCenterPanel project={project} onlineOptional={platform?.mode === "online-optional"} />, "Releases": <GitHubCenterPanel project={project} onlineOptional={platform?.mode === "online-optional"} />,
    "Settings": <LocalPlatformPanel platform={platform} onModeChange={onPlatformModeChange} />, "Trust": <TrustPanel project={project} onTrust={onTrust} />, "Permissions": <PermissionsPanel project={project} />, "Storage": <StoragePanel project={project} />, "Offline / Online": <LocalPlatformPanel platform={platform} onModeChange={onPlatformModeChange} />, "System diagnostics": <SystemDiagnosticsPanel platform={platform} />,
  };
...
function PreviewPanel({ project }: { project: ProjectSummary }) {
  const [preview, setPreview] = useState<{ state: string; command?: string; port?: number; url?: string; pid?: number; startedAt?: string; stoppedAt?: string; checkedAt?: string; health?: { ok: boolean; status?: number; detail: string }; logs: string[]; error?: string } | null>(null);
  const [message, setMessage] = useState("Reading local Preview state…");
  const request = async (path: string, method = "GET") => { try { const response = await fetch(`/api/workspace/projects/${project.id}/preview${path}`, { method }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || payload.preview?.error || "Preview operation failed."); setPreview(payload.preview); setMessage(""); return payload.preview; } catch (cause: unknown) { setMessage(cause instanceof Error ? cause.message : "Preview operation failed."); return null; } };
  const refresh = async () => { await request(""); };
  useEffect(() => { void refresh(); }, [project.id]);
  const running = preview?.state === "running" || preview?.state === "starting";
  return <section className="kf-capability-panel"><div className="kf-card-heading"><div><Play size={17} /><h3>Local Project Preview</h3></div><button onClick={() => void refresh()}>Refresh</button></div><p className="kf-capability-copy">Preview starts only a detected local preview, dev, or start script after the selected project is trusted. KForge captures process logs and verifies the local HTTP route; it does not deploy or expose the project publicly. Browser console and network telemetry are not captured, and Preview errors are not automatically created as Problems or Tasks.</p>{message && <StatusMessage>{message}</StatusMessage>}<div className="kf-hardware-grid"><span><strong>State</strong>{preview?.state || "loading"}</span><span><strong>Health</strong>{preview?.health?.ok ? `HTTP ${preview.health.status}` : preview?.health?.detail || "not checked"}</span><span><strong>Last HTTP check</strong>{preview?.checkedAt ? formatDate(preview.checkedAt) : "not checked"}</span><span><strong>Port</strong>{preview?.port || "not allocated"}</span><span><strong>Trust</strong>{project.trust}</span></div><div className="kf-inline-controls"><button className="kf-button kf-button--primary" disabled={project.trust !== "trusted" || running} onClick={() => void request("/start", "POST")}>Start Preview</button><button disabled={project.trust !== "trusted" || !running} onClick={() => void request("/restart", "POST")}>Restart</button><button disabled={!running} onClick={() => void request("/health", "POST")}>Health check</button><button disabled={project.trust !== "trusted" || !running} onClick={() => void request("/stop", "POST")}>Stop</button></div>{project.trust !== "trusted" && <p className="kf-capability-copy">BLOCKED: trust this local project before starting, restarting, or stopping a project process.</p>}{preview?.command && <article className="kf-command-evidence"><strong>Detected command</strong><small>{preview.command}</small><small>{preview.url || "No local URL was allocated."}</small></article>}{preview?.url && preview.health?.ok && <iframe className="kf-preview-frame" title={`Local Preview: ${project.name}`} src={preview.url} />}{preview?.error && <article className="kf-command-evidence"><strong>Preview error</strong><pre>{preview.error}</pre></article>}<article className="kf-command-evidence"><strong>Process logs</strong><pre>{preview?.logs?.length ? preview.logs.join("\n") : "No Preview process log has been captured in this KForge server session."}</pre></article></section>;
}
...
function CodeUnderstandingPanel({ project }: { project: ProjectSummary }) {
  ...
  useEffect(() => { void refresh(); }, [project.id]);
  return <section className="kf-capability-panel"><div className="kf-card-heading"><div><Code2 size={17} /><h3>Code Understanding</h3></div><button onClick={() => void refresh()}>Refresh evidence</button></div>{message && <StatusMessage>{message}</StatusMessage>}{data?.profile && <><div className="kf-hardware-grid"><span><strong>Workspace</strong>{data.profile.workspaceKind}</span><span><strong>Source files</strong>{data.profile.sourceFileCount}</span><span><strong>Graph imports</strong>{data.graph?.summary.imports ?? "Unavailable"}</span><span><strong>Routes</strong>{data.graph?.summary.routes ?? "Unavailable"}</span><span><strong>APIs</strong>{data.graph?.summary.apis ?? "Unavailable"}</span></div><article className="kf-command-evidence"><strong>Measured project structure</strong><pre>{JSON.stringify({ frameworks: data.profile.framework, languages: data.profile.languages, sourceRoots: data.profile.sourceRoots, testRoots: data.profile.testRoots, runtimeEntrypoint: data.profile.runtimeEntrypoint || "UNAVAILABLE" }, null, 2)}</pre></article></>}</section>;
}
...
function MarketplacePanel() {
  ...
  const installModel = async (item: MarketplaceItemView) => { const model = item.id.replace(/^ollama:/, ""); if (item.category !== "models" || item.installAction !== "INSTALL_REQUIRES_CONFIRMATION") { setMessage("This Marketplace item has no verified installation adapter."); return; } if (!window.confirm(`Download ${model} from the configured local-model provider after reviewing permissions and requirements?`)) return; try { const response = await fetch("/api/workspace/ai/models/install", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ provider: "ollama", model, confirmed: true }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || "Model installation could not start."); setMessage(`Verified installation task ${payload.task?.id || "started"} is running. Open Tasks for real progress and evidence.`); } catch (cause: unknown) { setMessage(cause instanceof Error ? cause.message : "Marketplace installation failed."); } };
  const activateModel = async (item: MarketplaceItemView) => { const model = item.id.replace(/^ollama:/, ""); try { const response = await fetch("/api/workspace/ai/models/active", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ provider: "ollama", model }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || "Model activation failed."); setMessage(`Local model ${model} is now active.`); await load(); } catch (cause: unknown) { setMessage(cause instanceof Error ? cause.message : "Model activation failed."); } };
  const removeModel = async (item: MarketplaceItemView) => { const model = item.id.replace(/^ollama:/, ""); if (!window.confirm(`Remove installed local model ${model}?`)) return; try { const response = await fetch(`/api/workspace/ai/models/${encodeURIComponent(model)}`, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ provider: "ollama", confirmed: true }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || "Model removal failed."); setMessage(payload.message || `Removed ${model}.`); setSelected(null); await load(); } catch (cause: unknown) { setMessage(cause instanceof Error ? cause.message : "Model removal failed."); } };
  ...
  return <section className="kf-capability-panel"><div className="kf-card-heading"><div><Box size={17} /><h3>Knoux Forge Marketplace</h3></div><button onClick={() => void load()}>Refresh Marketplace evidence</button></div><p className="kf-capability-copy">Marketplace data is sourced from local runtime metadata or a declared official provider. It never invents ratings, prices, downloads, or install state. Remote installs remain disabled in Offline Mode and always require review.</p><div className="kf-provider-grid">{providers.map((provider) => <article className="kf-provider-card" key={provider.id}><div><strong>{provider.label}</strong><span>{provider.state}</span></div><p>{provider.detail}</p><small>{provider.adapterKind || "registry"} adapter · {provider.configured ? "configured" : "not configured"}</small><small>Capabilities: {(provider.capabilities || []).join(", ") || "not declared"}</small>{provider.sourceUrl && <a href={provider.sourceUrl} target="_blank" rel="noreferrer">Source</a>}</article>)}</div><div className="kf-inline-controls"><input aria-label="Search Marketplace" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search models and capabilities" /><select aria-label="Marketplace category" value={filter} onChange={(event) => setFilter(event.target.value)}><option value="all">All categories</option><option value="models">Models</option><option value="agents">Agents</option><option value="tools">Tools</option><option value="plugins">Plugins</option></select></div>{message && <StatusMessage>{message}</StatusMessage>}<div className="kf-provider-grid">{visible.map((item) => <article className="kf-provider-card" key={item.id}><div><strong>{item.name}</strong><span>{item.enabled ? "ACTIVE_LOCAL" : item.installed ? "INSTALLED_LOCAL" : item.dataState}</span></div><p>{item.description}</p><small>{item.source} · {item.version || "version not supplied"} · {item.license || "license not supplied"}</small><small>State: {item.enabled ? "active local runtime" : item.installed ? "installed locally" : "not installed"} · Source data: {item.dataState}</small><small>Trust: {item.trust} · {item.compatibility}</small><div className="kf-inline-controls"><button onClick={() => setSelected(item)}>Open details</button><button onClick={() => void inspect(item)}>{item.installed ? "Inspect local item" : "Review install"}</button>{item.sourceUrl && <a href={item.sourceUrl} target="_blank" rel="noreferrer">Source</a>}</div><details><summary>Capabilities, requirements & permissions</summary><pre>{JSON.stringify({ capabilities: item.capabilities, requirements: item.requirements, permissions: item.permissions }, null, 2)}</pre></details></article>)}</div>{selected && <article className="kf-command-evidence"><div className="kf-card-heading"><div><Box size={17} /><h3>{selected.name}</h3></div><button onClick={() => setSelected(null)}>Close details</button></div><p>{selected.description}</p><pre>{JSON.stringify({ category: selected.category, source: selected.source, version: selected.version || "DATA_UNAVAILABLE", license: selected.license || "DATA_UNAVAILABLE", compatibility: selected.compatibility, capabilities: selected.capabilities, requirements: selected.requirements, permissions: selected.permissions, trust: selected.trust, installed: selected.installed, enabled: selected.enabled, dataState: selected.dataState }, null, 2)}</pre><div className="kf-inline-controls">{selected.category === "models" && !selected.installed && <button className="kf-button kf-button--primary" disabled={selected.installAction !== "INSTALL_REQUIRES_CONFIRMATION"} onClick={() => void installModel(selected)}>Install with confirmation</button>}{selected.category === "models" && selected.installed && <button onClick={() => void activateModel(selected)}>Activate local model</button>}{selected.category === "models" && selected.installed && <button onClick={() => void removeModel(selected)}>Remove local model</button>}{selected.sourceUrl && <a href={selected.sourceUrl} target="_blank" rel="noreferrer">Official source</a>}</div></article>}{!visible.length && <p className="kf-capability-copy">No configured Marketplace item matches the current search and filters.</p>}</section>;
}
