...
function OnlineHubPanel({ initialView, project, platform }: { initialView: OnlineHubView; project: ProjectSummary; platform?: LocalPlatformStatus }) {
  ...
  const load = async () => {
    try {
      const [response, taskResponse, controlResponse] = await Promise.all([fetch("/api/workspace/marketplace"), fetch("/api/workspace/tasks"), fetch(`/api/workspace/projects/${project.id}/online/control-center`)]);
      const payload = await response.json();
