...
function CapabilitySurface({ activeNav, project, projects, scan, tasks, results, platform, settings, searchSelection, onSettingsChange, onPlatformModeChange, onOpenProject, onRun, onTaskControl, onTrust, onNavigate, onRefreshWorkspace, onProjectCollectionUpdate }: { activeNav: string; project: ProjectSummary; projects: ProjectSummary[]; scan?: ProjectScan; tasks: TaskItem[]; results?: Partial<Record<WorkspaceAction, CommandResult>>; platform?: LocalPlatformStatus; settings: KForgePlatformSettings | null; searchSelection?: GlobalSearchResult; onSettingsChange: (settings: KForgePlatformSettings) => void; onPlatformModeChange: (mode: LocalPlatformStatus["mode"]) => void; onOpenProject: () => void; onRun: (action: WorkspaceAction) => void; onTaskControl: (task: TaskItem, control: "cancel" | "retry" | "resume" | "rollback") => void; onTrust: () => void; onNavigate: (label: string) => void; onRefreshWorkspace: () => void; onProjectCollectionUpdate: (project: ProjectSummary, patch: Partial<Pick<ProjectSummary, "favorite" | "pinned" | "archived" | "tags">>) => void }) {
  const renderers: Record<string, ReactNode> = {
    "Workspace": <CollectionProjectsPanel title="Workspace Projects" projects={projects.filter((entry) => !entry.archived)} empty="No local project is currently available in this workspace." icon={<LayoutDashboard size={17} />} />, "Project health": <ProjectHealthPanel project={project} />, "Recent projects": <RecentProjectsPanel projects={projects} />, "Favorites": <CollectionProjectsPanel title="Favorite Projects" projects={projects.filter((entry) => entry.categories.favorite)} empty="No projects have been marked as favorite in the local workspace." icon={<Star size={17} />} collection="favorite" onCollectionUpdate={onProjectCollectionUpdate} />, "Pinned": <CollectionProjectsPanel title="Pinned Projects" projects={projects.filter((entry) => entry.categories.pinned && !entry.archived)} empty="No projects have been pinned in the local workspace." icon={<Pin size={17} />} collection="pinned" onCollectionUpdate={onProjectCollectionUpdate} />, "Archive": <CollectionProjectsPanel title="Archived Projects" projects={projects.filter((entry) => entry.categories.archive)} empty="No projects have been archived in the local workspace." icon={<Archive size={17} />} collection="archive" onCollectionUpdate={onProjectCollectionUpdate} />, "Open project": <OpenProjectPanel onOpen={onOpenProject} />, "Import project": <OpenProjectPanel onOpen={onOpenProject} />,
...
function CollectionProjectsPanel({ title, projects, empty, icon, collection, onCollectionUpdate }: { title: string; projects: ProjectSummary[]; empty: string; icon: React.ReactNode; collection?: "favorite" | "pinned" | "archive"; onCollectionUpdate?: (project: ProjectSummary, patch: Partial<Pick<ProjectSummary, "favorite" | "pinned" | "archived" | "tags">>) => void }) {
  const action = collection === "favorite" ? { label: "Remove favorite", patch: { favorite: false } } : collection === "pinned" ? { label: "Unpin project", patch: { pinned: false } } : collection === "archive" ? { label: "Restore from archive", patch: { archived: false } } : null;
  return <section className="kf-capability-panel"><div className="kf-card-heading"><div>{icon}<h3>{title}</h3></div><span>{projects.length} local record(s)</span></div>{projects.length ? <div className="kf-provider-grid">{projects.map((entry) => <article className="kf-provider-card" key={entry.id}><div><strong>{entry.name}</strong><span>{entry.projectType}</span></div><p>{entry.path}</p><small>{entry.branch} · {formatDate(entry.lastOpenedAt || entry.lastActivity)} · {entry.trust === "trusted" ? "trusted" : "read-only"}</small>{action && onCollectionUpdate && <button type="button" className="kf-button kf-button--ghost" onClick={() => onCollectionUpdate(entry, action.patch)}>{action.label}</button>}</article>)}</div> : <p className="kf-capability-copy">{empty}</p>}</section>;
}
...
function SystemDiagnosticsPanel({ platform, onRefreshWorkspace }: { platform?: LocalPlatformStatus; onRefreshWorkspace: () => void }) {
  return <section className="kf-capability-panel"><div className="kf-card-heading"><div><Activity size={17} /><h3>System Diagnostics</h3></div><span>{platform?.mode || "Loading"}</span></div>{platform ? <><p className="kf-capability-copy">Diagnostics describe local capabilities and their measured availability. Missing tools remain unavailable rather than being represented as successful.</p><div className="kf-inline-controls"><button type="button" onClick={onRefreshWorkspace}>Refresh diagnostics</button></div><div className="kf-provider-grid">{platform.capabilities.map((capability) => <article className="kf-provider-card" key={capability.id}><div><strong>{capability.label}</strong><span>{capability.state}</span></div><small>{capability.detail}</small></article>)}</div></> : <p className="kf-capability-copy">Loading local platform diagnostics…</p>}</section>;
}
...
function GitCenterPanel({ project }: { project: ProjectSummary }) {
  const [data, setData] = useState<{ branch?: string; remoteUrl?: string; status?: string; diffStat?: string; stagedDiffStat?: string; changes?: Array<{ file: string; indexStatus: string; worktreeStatus: string; staged: boolean; unstaged: boolean; untracked: boolean }>; branches?: string[]; commits?: Array<{ sha: string; shortSha: string; subject: string; committedAt: string }>; stashes?: string[]; tags?: string[] } | null>(null);
  ...
  useEffect(() => { void refresh(); setProposal(null); setPrePush(null); setSelectedFiles([]); setCommitMessage(""); }, [project.id]);
  ...
  const updateIndex = async (action: "stage" | "unstage") => {
    if (project.trust !== "trusted") { setMessage("UNTRUSTED PROJECT: Git index changes require local trust approval."); return; }
    if (!selectedFiles.length) { setMessage("Select one or more project-relative files before changing the Git index."); return; }
    const verb = action === "stage" ? "stage" : "unstage";
    if (!window.confirm(`${verb === "stage" ? "Stage" : "Unstage"} ${selectedFiles.length} selected local file(s)? ${verb === "unstage" ? "Working-tree contents are preserved." : "No remote operation will occur."}`)) return;
    try { const response = await fetch(`/api/workspace/projects/${project.id}/git/${action}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ files: selectedFiles, confirmed: true }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || `Git ${verb} failed.`); setMessage(`${selectedFiles.length} file(s) ${verb}d locally. No remote operation was performed.`); setSelectedFiles([]); await refresh(); } catch (cause: unknown) { setMessage(cause instanceof Error ? cause.message : `Git ${verb} failed.`); }
  };
  const commitLocal = async () => {
    const messageText = commitMessage.trim();
    if (project.trust !== "trusted") { setMessage("UNTRUSTED PROJECT: a local commit requires local trust approval."); return; }
    if (!messageText) { setMessage("Enter a local commit message before committing staged files."); return; }
    if (!window.confirm(`Create a local Git commit with message: ${messageText}? This operation does not push to a remote.`)) return;
    try { const response = await fetch(`/api/workspace/projects/${project.id}/git/commit`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: messageText, confirmed: true }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || "Local commit failed."); setCommitMessage(""); setSelectedFiles([]); setMessage("Local commit created. Remote push: NOT_PERFORMED."); await refresh(); } catch (cause: unknown) { setMessage(cause instanceof Error ? cause.message : "Local commit failed."); }
  };
  const changes = data?.changes || [];
  const selectedChanges = changes.filter((change) => selectedFiles.includes(change.file));
  const canStage = selectedChanges.some((change) => !change.staged);
  const canUnstage = selectedChanges.some((change) => change.staged);
  return <section className="kf-capability-panel"><div className="kf-card-heading"><div><GitBranch size={17} /><h3>Git Center</h3></div><button onClick={() => void refresh()}>Refresh</button></div>{message && <StatusMessage>{message}</StatusMessage>}{data && <><div className="kf-hardware-grid"><span><strong>Branch</strong>{data.branch || "Unavailable"}</span><span><strong>Remote</strong>{data.remoteUrl || "No remote"}</span><span><strong>Stashes</strong>{data.stashes?.length || 0}</span><span><strong>Tags</strong>{data.tags?.length || 0}</span></div><article className="kf-command-evidence"><strong>Working tree</strong><pre>{data.status || "No Git status output."}</pre></article><article className="kf-command-evidence"><strong>Unstaged diff stat</strong><pre>{data.diffStat || "Clean working tree."}</pre></article><article className="kf-command-evidence"><strong>Staged diff stat</strong><pre>{data.stagedDiffStat || "No staged changes."}</pre></article><article className="kf-command-evidence"><strong>Local index controls</strong><small>Only selected project-relative files are sent to Git. Every index or commit write requires explicit confirmation. Remote push is not available here.</small>{changes.length ? <><div className="kf-git-file-list">{changes.map((change) => <label key={change.file}><input type="checkbox" checked={selectedFiles.includes(change.file)} onChange={(event) => setSelectedFiles((current) => event.target.checked ? [...new Set([...current, change.file])] : current.filter((file) => file !== change.file))} /><code>{change.indexStatus}{change.worktreeStatus}</code><span>{change.file}</span><small>{change.untracked ? "untracked" : change.staged && change.unstaged ? "staged + unstaged" : change.staged ? "staged" : "unstaged"}</small></label>)}</div><div className="kf-inline-controls"><button disabled={project.trust !== "trusted" || !canStage} onClick={() => void updateIndex("stage")}>Stage selected</button><button disabled={project.trust !== "trusted" || !canUnstage} onClick={() => void updateIndex("unstage")}>Unstage selected</button></div></> : <p>No changed files are available for staging.</p>}<div className="kf-inline-controls"><input value={commitMessage} maxLength={240} onChange={(event) => setCommitMessage(event.target.value)} placeholder="Local commit message" aria-label="Local commit message" /><button disabled={project.trust !== "trusted" || !changes.some((change) => change.staged)} onClick={() => void commitLocal()}>Create local commit</button></div>{project.trust !== "trusted" && <p>UNTRUSTED PROJECT: approve local execution before changing the Git index or creating a commit.</p>}</article><div className="kf-inline-controls"><input value={branchName} onChange={(event) => setBranchName(event.target.value)} placeholder="feature/branch-name" aria-label="New local branch name" /><button onClick={() => void createBranch()}>Create branch</button><button onClick={() => void loadProposal()}>Smart Commit preview</button><button onClick={() => void runPrePush()}>Run Pre-push Gate</button></div>{proposal && <article className="kf-command-evidence"><strong>{proposal.title}</strong><small>{proposal.description}</small><pre>{JSON.stringify({ changedFiles: proposal.changedFiles, diffStat: proposal.diffStat, validations: proposal.validations }, null, 2)}</pre></article>}{prePush && <article className="kf-command-evidence"><strong>Pre-push verification</strong><pre>{JSON.stringify(prePush, null, 2)}</pre></article>}<div className="kf-provider-grid">{(data.commits || []).slice(0, 8).map((commit) => <article className="kf-provider-card" key={commit.sha}><div><strong>{commit.subject}</strong><span>{commit.shortSha}</span></div><small>{formatDate(commit.committedAt)}</small></article>)}{(data.tags || []).slice(0, 8).map((tag) => <article className="kf-provider-card" key={`tag-${tag}`}><div><strong>{tag}</strong><span>Tag</span></div></article>)}</div></>}</section>;
}
...
function GraphPanel({ project, initialTarget }: { project: ProjectSummary; initialTarget?: string }) {
  ...
  const graphMapNodes = (graph?.nodes || []).slice(0, 72).map((node, index) => ({ node, x: 82 + (index % 6) * 166, y: 56 + Math.floor(index / 6) * 74 }));
  const graphMapPositions = new Map(graphMapNodes.map((entry) => [entry.node.id, entry]));
  const graphMapEdges = (graph?.edges || []).filter((edge) => graphMapPositions.has(edge.from) && graphMapPositions.has(edge.to)).slice(0, 120);
  const graphMapHeight = Math.max(180, 80 + Math.ceil(graphMapNodes.length / 6) * 74);
  return <section className="kf-capability-panel">
    ...
    {graph ? <>
      ...
      <section className="kf-graph-map" aria-label="Interactive local project graph"><header><div><strong>Interactive local project graph</strong><small>{graphMapNodes.length} rendered node(s) · {graphMapEdges.length} rendered edge(s) · selected node fills the impact target</small></div><span>LOCAL STATIC EVIDENCE</span></header>{graphMapNodes.length ? <svg viewBox={`0 0 1000 ${graphMapHeight}`} role="img" aria-label="Local static graph map"><defs><marker id="kf-graph-arrow" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0,0 L0,7 L7,3.5 z" /></marker></defs>{graphMapEdges.map((edge) => { const from = graphMapPositions.get(edge.from)!; const to = graphMapPositions.get(edge.to)!; return <line key={`${edge.from}:${edge.to}:${edge.type}`} className="kf-graph-map-edge" x1={from.x} y1={from.y} x2={to.x} y2={to.y} markerEnd="url(#kf-graph-arrow)" aria-label={`${edge.type}: ${from.node.label} to ${to.node.label}`} />; })}{graphMapNodes.map(({ node, x, y }) => <g key={node.id} className={`kf-graph-map-node kf-graph-map-node--${node.type}`} role="button" tabIndex={0} aria-label={`Select ${node.type} node ${node.label}`} onClick={() => setFile(node.path || node.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setFile(node.path || node.id); } }}><circle cx={x} cy={y} r="21" /><text x={x} y={y + 4} textAnchor="middle">{node.type.slice(0, 1).toUpperCase()}</text><title>{`${node.label} · ${node.type}${node.path ? ` · ${node.path}` : ""}`}</title></g>)}</svg> : <p className="kf-capability-copy">No local graph node is available within current evidence coverage.</p>}<div className="kf-graph-map-legend"><span><i className="kf-graph-map-node--file" />File / test</span><span><i className="kf-graph-map-node--symbol" />Symbol</span><span><i className="kf-graph-map-node--dependency" />Dependency</span><span>Click or press Enter on a node to stage it for impact analysis.</span></div></section>
      <h4>Files</h4>
...
function ReleasePreparationPanel({ project, view }: { project: ProjectSummary; view: "Release preparation" | "Artifacts" | "Versioning" }) {
  ...
  useEffect(() => { void refresh(); }, [project.id]);
  const value = view === "Artifacts" ? { artifacts: preparation?.artifacts, localEvidence: preparation?.localEvidence, notice: preparation?.notice } : view === "Versioning" ? { baselineTag: preparation?.baselineTag, version: preparation?.version } : preparation;
  return <section className="kf-capability-panel"><div className="kf-card-heading"><div><Rocket size={17} /><h3>{view}</h3></div><button onClick={() => void refresh()}>Refresh evidence</button></div><p className="kf-capability-copy">This is a local, read-only release preparation preview. Artifact evidence identifies its own source, digest, timestamp, and signature state; it never claims a CI artifact or creates tags, releases, or remote requests.</p>{message && <StatusMessage>{message}</StatusMessage>}{preparation && <article className="kf-command-evidence"><strong>{view === "Artifacts" ? "Independent local artifact evidence" : "Local release evidence"}</strong><pre>{JSON.stringify(value, null, 2)}</pre></article>}</section>;
}
...
function OnlineHubPanel({ initialView, project, platform, initialSelectionId }: { initialView: OnlineHubView; project: ProjectSummary; platform?: LocalPlatformStatus; initialSelectionId?: string }) {
  ...
  const manageLocalPackage = async (item: MarketplaceItemView, action: "install" | "health" | "run" | "update" | "uninstall") => {
    const mutates = action !== "health";
    if (mutates && !window.confirm(`${action === "run" ? "Run" : action[0].toUpperCase() + action.slice(1)} local package ${item.name}?\n\nExecution: LOCAL\nNetwork: NOT_REQUIRED\nData: verified local package files and selected workspace state\nSecret redaction: enforced\nConfirmation: REQUIRED`)) return;
    try {
      const endpoint = `/api/workspace/marketplace/items/${encodeURIComponent(item.id)}/${action}`;
      const response = await fetch(endpoint, action === "health" ? undefined : { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ confirmed: true }) });
      const payload = await response.json() as { message?: string; stage?: string; error?: string; version?: string; output?: string };
      if (!response.ok) throw new Error(payload.error || payload.message || `Local package ${action} failed.`);
      const detail = payload.message || payload.stage || `${action} completed`;
      await load();
      setMessage(`Local package ${action} verified: ${detail}${payload.version ? ` · version ${payload.version}` : ""}${payload.output ? `\n${payload.output}` : ""}`);
    } catch (cause: unknown) { setMessage(cause instanceof Error ? cause.message : `Local package ${action} failed.`); }
  };
...
function MarketplaceItemDetail({ item, onInspect, onInstall, onActivate, onRemove, onPackageLifecycle }: { item: MarketplaceItemView; onInspect: (item: MarketplaceItemView) => Promise<void>; onInstall: (item: MarketplaceItemView) => Promise<void>; onActivate: (item: MarketplaceItemView) => Promise<void>; onRemove: (item: MarketplaceItemView) => Promise<void>; onPackageLifecycle: (item: MarketplaceItemView, action: "install" | "health" | "run" | "update" | "uninstall") => Promise<void> }) {
  return <>
    ...
    <div className="kf-online-detail-actions"><button onClick={() => void onInspect(item)}>Review install</button>{item.category === "models" && !item.installed && <button className="is-primary" disabled={item.installAction !== "INSTALL_REQUIRES_CONFIRMATION"} onClick={() => void onInstall(item)}>Install</button>}{item.category === "models" && item.installed && !item.enabled && <button className="is-primary" onClick={() => void onActivate(item)}>Activate</button>}{item.category === "models" && item.installed && <button onClick={() => void onRemove(item)}>Remove</button>}{item.id.startsWith("package:") && !item.installed && <button className="is-primary" disabled={item.installAction !== "INSTALL_REQUIRES_CONFIRMATION"} title={item.installAction === "INSTALL_REQUIRES_CONFIRMATION" ? "Installs the verified bundled package after confirmation." : "No verified local package install adapter is available."} onClick={() => void onPackageLifecycle(item, "install")}>Install local package</button>}{item.id.startsWith("package:") && item.installed && <><button onClick={() => void onPackageLifecycle(item, "health")}>Health check</button><button onClick={() => void onPackageLifecycle(item, "run")}>Run local package</button>{item.updateState.value?.startsWith("UPDATE_AVAILABLE") ? <button onClick={() => void onPackageLifecycle(item, "update")}>Update local package</button> : <span className="kf-capability-copy">Update: {item.updateState.value || item.updateState.state} · no update action is currently available.</span>}<button onClick={() => void onPackageLifecycle(item, "uninstall")}>Uninstall local package</button></>}{item.sourceUrl && <a href={item.sourceUrl} target="_blank" rel="noreferrer">Official source</a>}</div>
  </>;
}
