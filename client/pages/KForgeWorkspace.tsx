...
function ProjectHealthPanel({ project }: { project: ProjectSummary }) {
  const [data, setData] = useState<{ health: { score: number | null; evidenceCoverage: number; calculatedAt: string; metrics: Array<{ key: string; label: string; status: WorkspaceStatus; score: number | null; weight: number; evidence: string[]; findings: string[]; lastScan: string }>; release: { state: string; blockers: Array<{ title: string; source: string; file?: string }>; warnings: Array<{ title: string; source: string; file?: string }>; evidence: string[] } }; scannedAt: string; issueCount: number; tools: Array<{ name: string; available: boolean; version?: string; reason?: string }> } | null>(null);
...
function AgentMissionCenter({ project }: { project: ProjectSummary }) {
  ...
  return <section className="kf-capability-panel"><div className="kf-card-heading"><div><Bot size={17} /><h3>KForge Engineer missions</h3></div><button onClick={() => void refreshRegistry()}>Refresh tools</button></div><p className="kf-capability-copy">Read · Plan · Patch · Verify. The agent only invokes registered typed tools; commits, pushes, deployment, force push, and secret exposure remain permission-gated or blocked.</p><div className="kf-inline-controls"><select value={mission} onChange={(event) => setMission(event.target.value)}><option value="audit">Audit project</option><option value="fix-critical">Fix critical issue</option><option value="prepare-release">Prepare production release</option></select><button className="kf-button kf-button--primary" onClick={() => void start()}>Start mission</button></div><p className="kf-capability-copy">{status}</p>{registry && <><div className="kf-provider-grid">{(registry.tools || []).map((tool) => <article className="kf-provider-card" key={tool.id}><div><strong>{tool.label}</strong><span>{tool.permission}</span></div><p>{tool.available ? "Available" : "Unavailable"}</p><small>{tool.reason || "Registered local tool"}</small></article>)}</div><article className="kf-command-evidence"><strong>Agent permissions</strong><pre>{JSON.stringify(registry.permissions, null, 2)}</pre></article></>}</section>;
}
