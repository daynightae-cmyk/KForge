...
function OnlineSurface({ view, project, onInspectorContext }: SurfaceProps) {
  ...
  const actionsByItemId = useMemo(() => {
    return new Map([...items, ...mcpItems, ...ovsxItems, ...hfItems, ...npmItems, ...pypiItems, ...nugetItems].map((item) => [item.id, lifecycleActions(item, operate, selectItem)]));
  }, [items, mcpItems, ovsxItems, hfItems, npmItems, pypiItems, nugetItems, operate, selectItem]);
