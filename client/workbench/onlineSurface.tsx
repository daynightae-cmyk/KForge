...
function OnlineSurface({ view, project, onInspectorContext }: SurfaceProps) {
  ...
  const selected = useMemo(() => items.find((item) => item.id === selectedId) || mcpItems.find((item) => item.id === selectedId) || ovsxItems.find((item) => item.id === selectedId) || hfItems.find((item) => item.id === selectedId) || npmItems.find((item) => item.id === selectedId) || pypiItems.find((item) => item.id === selectedId) || items[0] || null, [items, mcpItems, ovsxItems, hfItems, npmItems, pypiItems, selectedId]);
  ...
  const actionsByItemId = useMemo(() => {
    return new Map([...items, ...mcpItems, ...ovsxItems, ...hfItems, ...npmItems, ...pypiItems].map((item) => [item.id, lifecycleActions(item, operate, selectItem)]));
  }, [items, mcpItems, ovsxItems, hfItems, npmItems, pypiItems, operate, selectItem]);
