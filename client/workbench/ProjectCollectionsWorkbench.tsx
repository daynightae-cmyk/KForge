...
function ProjectCollectionsWorkbench({ view, workspace, project, onProjectSelect, onRefresh }: {
  ...
  const rows = useMemo(() => {
    ...
    result.sort((a, b) => sort === "name" ? a.name.localeCompare(b.name) : sort === "health" ? (b.healthScore ?? -1) - (a.healthScore ?? -1) : b.lastActivity.localeCompare(a.lastActivity));
    return result;
  }, [query, sort, view, workspace]);
