...
export function CanonicalInspector(props: CanonicalInspectorProps) {
  ...
  if (context?.kind === "online-item" && isMarketplaceItem(context.item)) {
    ...
    return <aside className="kw-inspector" aria-label="Online item details"><div className="kw-inspector-scroll" tabIndex={0}><div className="kw-inspector-title"><StatusBadge value={item.name} /><div><strong>{context.title || "Online item"}</strong><small>Online / {viewLabel("online", context.view || view)}</small></div></div><h2>Overview</h2><p>{String(item.description || item.overview || "No description supplied by verified metadata.")}</p><EvidenceRows value={{ category: item.category, version: item.version, health: item.healthState }} /><h2>Authority</h2><EvidenceRows value={{ authority: item.authority?.kind, originalAuthority: item.authority?.originalKind }} /><h2>Source</h2><EvidenceRows value={{ source: item.source }} /><h2>Availability</h2><EvidenceRows value={{ availability: item.availability, unavailableReason: item.unavailableReason }} /><h2>Runtime Evidence</h2><EvidenceRows value={{ runtimeEvidence: item.runtimeEvidence?.state, runtimeSources: item.runtimeEvidence?.sources?.join(", ") }} /><h2>Compatibility</h2><EvidenceRows value={{ state: context.compatibility || (project ? item.projectCompatibility?.state : "NOT_EVALUATED"), project: context.projectName || (project?.name || "No project selected"), source: item.projectCompatibility?.source }} /><h2>Permissions</h2>{(item.permissions ?? []).filter((p) => p.required).length ? <ul className="kw-contract">{(item.permissions ?? []).filter((p) => p.required).map((permission) => <li key={permission.id}><strong>{permission.id}</strong> — {permission.detail}</li>)}</ul> : <p className="kw-muted">No required permission was declared by verified metadata.</p>}<h2>Trust</h2><EvidenceRows value={{ trust: item.trust }} /><h2>Integrity</h2><EvidenceRows value={{ integrity: item.integrity?.state, value: item.integrity?.value, source: item.integrity?.source }} /><h2>Freshness</h2><EvidenceRows value={{ freshness: item.freshness?.state, checkedAt: item.freshness?.at }} /><h2>Lifecycle</h2>{(item.lifecycle ?? []).length ? (item.lifecycle ?? []).slice(0, 12).map((stage) => <div className="kw-lifecycle" key={stage.id}><StatusBadge value={stage.state} /><span>{stage.label}</span></div>) : <p className="kw-muted">No lifecycle evidence is available.</p>}<h2>Actions</h2><div className="kw-operation-actions">{actions.map((action) => <button key={action.id} disabled={action.disabled} title={action.reason} onClick={() => action.invoke?.()}>{action.label}</button>)}</div>{context.operation && <div className="kw-operation-result" role="status"><pre>{JSON.stringify(context.operation, null, 2)}</pre></div>}</div></aside>;
  }
