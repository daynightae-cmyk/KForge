...
function ProjectsSurface({ view, workspace, project, onProjectSelect, onRefresh }: SurfaceProps) {
  ...
  const rows = useMemo(() => {
    ...
    list.sort((a, b) => sort === "name" ? a.name.localeCompare(b.name) : sort === "health" ? (b.healthScore ?? -1) - (a.healthScore ?? -1) : b.lastActivity.localeCompare(a.lastActivity));
    return list;
  }, [query, sort, workspace]);
  ...
  return <section className="kw-projects" aria-label="KForge Projects Workspace">
    ...
    {rows.length ? <div className="kw-table-wrap" aria-label="Projects engineering table"><table className="kw-table" data-testid="project-table"><thead><tr><th>Bulk</th><th>Project</th><th>Type</th><th>Branch</th><th>Trust</th><th>Collections</th><th>Git</th><th>Health</th><th>Last Activity</th></tr></thead><tbody>{rows.map((entry) => <tr key={entry.id} data-project-path={entry.path} data-project-id={entry.id} className={project?.id === entry.id ? "is-selected" : ""}><td><input type="checkbox" aria-label={`Select ${entry.name} for bulk actions`} checked={selected.includes(entry.id)} onChange={(event) => setSelected((current) => event.target.checked ? [...new Set([...current, entry.id])] : current.filter((id) => id !== entry.id))} /></td><td><button className="kw-project-select" aria-label={`Select project ${entry.name}`} onClick={() => onProjectSelect(entry.id)}><strong>{entry.name}</strong><small>{entry.path}</small>{entry.tags.length ? <small>{entry.tags.join(" · ")}</small> : null}</button></td><td>{entry.projectType}</td><td><code>{entry.branch}</code></td><td><StatusBadge value={entry.trust} /></td><td><span>{entry.favorite ? "Favorite" : "Standard"}</span><small>{entry.pinned ? "Pinned" : "Not pinned"}</small></td><td><span>{entry.modifiedFiles + entry.untrackedFiles} changed</span><small>{entry.ahead} ahead · {entry.behind} behind</small></td><td>{entry.healthScore ?? "NOT_SCANNED"}</td><td>{new Date(entry.lastActivity).toLocaleString()}</td></tr>)}</tbody></table></div> : <EmptyState title="No projects found" detail="Open or import a project to populate this engineering table." />}
  </section>;
}
