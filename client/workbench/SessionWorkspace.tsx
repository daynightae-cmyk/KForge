    ...
    source.onmessage = (raw) => {
      try {
        ...
        if (["PATCH_PROPOSED", "PATCH_APPLIED", "APPROVAL_REQUIRED", "SESSION_COMPLETED", "ERROR", "PREVIEW_STATE", "USAGE_UPDATE"].includes(event.type)) {
          ...
          if (event.type.startsWith("PATCH_") || event.type === "APPROVAL_REQUIRED") void refreshPatches().catch(() => undefined);
        }
      } catch {
        /* malformed event data is ignored; the persisted endpoint remains authoritative */
      }
    };
    ...
    <div className="kw-ai-session" data-testid="ai-engineering-session">
      <header className="kw-ai-session-head">
        <div className="kw-ai-session-title">
          ...
          <div>
            <p>KNOuX / AI / Engineering Session</p>
            <h2>{session.task}</h2>
            <small>{project?.name || context?.project.name || session.projectId || "No project"} · {session.providerId}/{session.modelId}</small>
          </div>
        </div>
        <div className="kw-ai-session-state">
          <StatusBadge value={session.status} />
          <span>Autonomy <strong>{session.autonomy}</strong></span>
          <span>Checkpoint <strong>{session.checkpointId ? "READY" : "NONE"}</strong></span>
        </div>
        <div className="kw-row">
          ...
          <button disabled={busy || ["RUNNING", "PLANNING", "TOOL_EXECUTING", "VERIFYING", "PREVIEW_STARTING"].includes(session.status)} onClick={() => void start()}>{session.status === "READY" || session.status === "WAITING_FOR_APPROVAL" ? "Start / Resume" : "Run again"}</button>
      ...
      {session.pendingApproval && (
        <section className="kw-ai-approval" role="alert">
          <div><strong>Approval required</strong><span>{session.pendingApproval.summary}</span><code>{session.pendingApproval.id}</code></div>
          <div className="kw-row"><button disabled={busy} onClick={() => void approve()}>Approve</button><button disabled={busy} onClick={() => void reject()}>Reject</button></div>
        </section>
      )}
      ...
      <div className="kw-ai-session-grid">
        <aside className="kw-ai-context-pane">
          ...
          <dl className="kw-tech-dl">
            <div><dt>Project</dt><dd>{context?.project.name || project?.name || "UNKNOWN"}</dd></div>
            <div><dt>Branch</dt><dd>{context?.project.branch || project?.branch || "UNKNOWN"}</dd></div>
            <div><dt>Destination</dt><dd>{context?.destination || session.disclosureDestination || "UNKNOWN"}</dd></div>
            <div><dt>Files</dt><dd>{context?.filesIncluded.length ?? "UNKNOWN"}</dd></div>
            <div><dt>Est. tokens</dt><dd>{context?.estimatedTokens?.toLocaleString() ?? "UNKNOWN"}</dd></div>
            <div><dt>Source sent</dt><dd>{context ? (context.sourceCodeIncluded ? "YES" : "NO") : "UNKNOWN"}</dd></div>
          </dl>
          ...
          <div className="kw-context-files">
            {(context?.filesIncluded || []).map((file) => <button key={file.path} title={file.reason}><span>{file.path}</span><small>{file.reason} · {file.characters.toLocaleString()} chars</small></button>)}
          </div>
          <div className="kw-checkpoint-card">
            <strong>Checkpoint</strong>
            <code>{session.checkpointId || "Created automatically before first mutation"}</code>
            <small>{appliedCount} applied patch(es) · rollback requires explicit action.</small>
          </div>
        </aside>
        ...
        <main className="kw-ai-conversation-pane">
          ...
          <div className="kw-ai-event-stream" aria-live="polite">
            ...
            {modelEvents.map((event) => (
              <article key={event.id} className={`kw-ai-event kw-ai-event-${eventTone(event.type)}`}>
                <header><strong>{eventLabel(event.type)}</strong><time>{new Date(event.at).toLocaleTimeString()}</time></header>
                <p>{event.message}</p>
                {event.data && <details><summary>Evidence</summary><pre>{JSON.stringify(event.data, null, 2)}</pre></details>}
              </article>
            ))}
          </div>
          <div className="kw-ai-note-composer">
            <input value={operatorNote} onChange={(event) => setOperatorNote(event.target.value)} placeholder="Add an operator note to the auditable session…" onKeyDown={(event) => { if (event.key === "Enter" && operatorNote.trim()) void sendNote(); }} />
            <button disabled={busy || !operatorNote.trim()} onClick={() => void sendNote()}>Add note</button>
          </div>
          {patches.length > 0 && (
            <section className="kw-ai-diff-studio">
              <div className="kw-pane-heading"><strong>AI DIFF STUDIO</strong><span>{patches.length} patch(es)</span></div>
              {patches.map((patch) => (
                <details key={patch.id} open={patch.state === "PROPOSED"}>
                  <summary><span>{patch.file}</span><StatusBadge value={patch.state} /><small>{patch.risk}</small></summary>
                  <p>{patch.reason}</p>
                  <div className="kw-diff-columns"><pre aria-label="Before">{patch.oldText}</pre><pre aria-label="After">{patch.newText}</pre></div>
                  <div className="kw-row">
                    {patch.state === "PROPOSED" && <button disabled={busy || patch.risk === "blocked"} onClick={() => void applyPatch(patch)}>Apply through KForge</button>}
                    <code>{patch.id}</code>
                  </div>
                </details>
              ))}
        ...
        <aside className="kw-ai-inspector-pane">
          <nav className="kw-mini-tabs" aria-label="AI session inspector tabs">
            {(["PREVIEW", "MODEL", "CONTEXT", "DIAGNOSTICS", "SESSION"] as const).map((tab) => <button key={tab} className={rightTab === tab ? "is-active" : ""} onClick={() => setRightTab(tab)}>{tab}</button>)}
          </nav>
          {rightTab === "PREVIEW" && (
            <section className="kw-live-preview">
              <div className="kw-pane-heading"><strong>LIVE PREVIEW</strong><StatusBadge value={session.telemetry.previewState || (previewUrl ? "AVAILABLE" : "NOT_STARTED")} /></div>
              {previewUrl ? <><div className="kw-preview-toolbar"><code>{previewUrl}</code><a href={previewUrl} target="_blank" rel="noreferrer">Open ↗</a></div><iframe title="KForge live project Preview" src={previewUrl} sandbox="allow-scripts allow-forms allow-modals allow-popups allow-same-origin" /></> : <div className="kw-ai-empty"><strong>Preview not started</strong><span>Enable Preview after verification or let the model request the canonical start_preview tool. No screenshot or fake Preview is shown.</span></div>}
            </section>
          )}
          {rightTab === "MODEL" && (
            <section className="kw-inspector-stack">
              <h3>Model runtime</h3>
              <dl className="kw-tech-dl"><div><dt>Provider</dt><dd>{provider?.name || session.providerId}</dd></div><div><dt>Model</dt><dd>{session.modelId}</dd></div><div><dt>Status</dt><dd>{session.status}</dd></div><div><dt>TTFT</dt><dd>{session.telemetry.ttftMs === null ? "UNKNOWN" : `${session.telemetry.ttftMs}ms`}</dd></div><div><dt>Latency</dt><dd>{session.telemetry.latencyMs === null ? "UNKNOWN" : `${session.telemetry.latencyMs}ms`}</dd></div><div><dt>Tokens</dt><dd>{session.telemetry.totalTokens ?? "UNKNOWN"}</dd></div><div><dt>Tools</dt><dd>{session.telemetry.toolCalls}</dd></div><div><dt>Files</dt><dd>{session.telemetry.filesChanged}</dd></div></dl>
              <h4>Switch model</h4>
              <label className="kw-field">Provider<select value={switchProvider} onChange={(event) => { setSwitchProvider(event.target.value); setSwitchDisclosure(false); }}>{providers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
              <label className="kw-field">Model<select value={switchModel} onChange={(event) => setSwitchModel(event.target.value)}>{switchModels.length ? switchModels.map((model) => <option key={model.id} value={model.id}>{model.displayName}</option>) : <option value={switchModel}>{switchModel || "No discovered model"}</option>}</select></label>
              <label className="kw-inline-check"><input type="checkbox" checked={switchDisclosure} onChange={(event) => setSwitchDisclosure(event.target.checked)} /> Confirm new cloud destination if required</label>
              <button disabled={busy || !switchModel} onClick={() => void switchActiveModel()}>Switch model</button>
            </section>
          )}
      ...
      <section className="kw-ai-bottom-pane">
        <nav className="kw-mini-tabs" aria-label="AI execution bottom pane">
          {(["EVENTS", "TESTS", "BUILD", "OUTPUT", "PROBLEMS"] as const).map((tab) => <button key={tab} className={bottomTab === tab ? "is-active" : ""} onClick={() => setBottomTab(tab)}>{tab}</button>)}
        </nav>
        {bottomTab === "EVENTS" && <div className="kw-bottom-log">{events.slice(-80).map((event) => <code key={event.id}>{new Date(event.at).toLocaleTimeString()} [{eventLabel(event.type)}] {event.message}</code>)}</div>}
        {bottomTab === "TESTS" && <div className="kw-bottom-log">{commandEvents.filter((event) => event.type === "TEST_RESULT").map((event) => <code key={event.id}>{event.message}</code>)}{session.telemetry.testState && <code>Session test state: {session.telemetry.testState}</code>}</div>}
        {bottomTab === "BUILD" && <div className="kw-bottom-log">{commandEvents.filter((event) => event.type === "BUILD_RESULT").map((event) => <code key={event.id}>{event.message}</code>)}{session.telemetry.buildState && <code>Session build state: {session.telemetry.buildState}</code>}</div>}
        {bottomTab === "OUTPUT" && <div className="kw-bottom-log">{commandEvents.map((event) => <code key={event.id}>{event.message}</code>)}</div>}
        {bottomTab === "PROBLEMS" && <div className="kw-bottom-log">{errorEvents.length ? errorEvents.map((event) => <code key={event.id}>{event.message}</code>) : <code>No session errors recorded.</code>}</div>}
      </section>
    </div>
  );
}
