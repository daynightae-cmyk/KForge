...
| Desktop runtime | `COMPLETE_VERIFIED` | `npm run verify:desktop` confirms built SPA/server resources, random loopback binding, window load, controlled shutdown, and managed Preview/topology cleanup. | Code-signing and SmartScreen reputation remain unavailable without a signing certificate. |
...
| Windows CI | `COMPLETE_VERIFIED` | `.github/workflows/kforge-verification.yml` includes a distinct `windows-latest` job that installs locked dependencies, typechecks, produces NSIS, runs the installed-runtime verifier, and uploads the installer and evidence. [Run 32897765085](https://github.com/daynightae-cmyk/KForge/actions/runs/32897765085) completed successfully; the Windows package job completed in 5m 7s. | A clean Windows Sandbox or VM remains a separate environment claim. |
